...
template <typename Indexer> class KeySetIterator {
public:
  ...
  bool operator==(KeySetIterator other) const { return idx == other.idx; }
...
template <typename Indexer> class KeySet {
public:
  KeySet() { bits.reset(); }
  ...
  iterator begin() const { return iterator(bits, nextSetIndex(bits, 0)); }
